...
def select_columns():
    ...
    if request.method == "POST":
        ...
        rate=[]
        llm_question = ["Connects program domain concpets with problem domain concepts", "Talks about possible exceptions", "Discusses alternative solutions", "Provides definition use and links to external comments", "Explains code concepts - algorithmic details","Provides analysis on code complexity"]
        for _ in group_list:
            ratings = []
            for llm in rating_candidates:
                for j in range(0,6):
                    ratings.append({str(llm) + "_" + str(llm_question[j]):"NONE"})
                ...
                for row_idx, rating_list in enumerate(ratings):
                    for entry in rating_list:
                        for col_name, value in entry.items():
                            ...
                            df.at[row_idx, col_name] = value or "None"
